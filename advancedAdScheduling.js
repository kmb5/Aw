...
function main() {
  ...
  var spreadsheetUrl = "https://docs.google.com/spreadsheets/d/1E14nYXaa7vZX8PUserJlh40uWujZhO_Dt_mGXBuqdgU/edit?usp=sharing";
  ...
  var runMobileBids = true;
    ...
    //Use true if you want to set tablet bid adjustments as well as ad schedules.
  //Use false to just set ad schedules.
  var runTabletBids = true;
  
     //Use true if you want to set desktop bid adjustments as well as ad schedules.
  //Use false to just set ad schedules.
  var runDesktopBids = true;
  ...
  if(runMobileBids){
    if (sheets.length < 2) {
      ...
      sheet.getRange("I4").setValue(thisHourMultiplier_Mobile);
      sheet.getRange("T4").setValue(totalMultiplier);
      ModifyMobileBidAdjustment(campaignIds, thisHourMultiplier_Mobile);
    }
  }
  
   // Change the tablet bid adjustment
  if(runTabletBids){
    if (sheets.length < 2) {
      Logger.log("Tablet ad schedule sheet was not found in the Google spreadsheet.");
    } else {
      var sheet = sheets[2];
      var data = sheet.getRange(scheduleRange).getValues();
      var thisHourMultiplier_Tablet = data[hour][dayOfWeek];
 
      if (thisHourMultiplier_Tablet.length === 0) {
        thisHourMultiplier_Tablet = -1;
      }
 
      if (isNaN(thisHourMultiplier_Tablet) || (thisHourMultiplier_Tablet < -0.9 && thisHourMultiplier_Tablet > -1) || thisHourMultiplier_Tablet > 3) {
        Logger.log("Tablet bid modifier '" + thisHourMultiplier_Tablet + "' for " + weekDays[dayOfWeek] + " " + hour + " is not valid.");
        thisHourMultiplier_Tablet = 0;
      }
 
      var totalMultiplier = ((1+thisHourMultiplier_Tablet)*(1+thisHourMultiplier))-1;
      sheet.getRange("I4").setValue(thisHourMultiplier_Tablet);
      sheet.getRange("T4").setValue(totalMultiplier);
      ModifyTabletBidAdjustment(campaignIds, thisHourMultiplier_Tablet);
    }
  }
  
     // Change the desktop bid adjustment
  if(runDesktopBids){
    if (sheets.length < 2) {
      Logger.log("Desktop ad schedule sheet was not found in the Google spreadsheet.");
    } else {
      var sheet = sheets[3];
      var data = sheet.getRange(scheduleRange).getValues();
      var thisHourMultiplier_Desktop = data[hour][dayOfWeek];
 
      if (thisHourMultiplier_Desktop.length === 0) {
        thisHourMultiplier_Desktop = -1;
      }
 
      if (isNaN(thisHourMultiplier_Desktop) || (thisHourMultiplier_Desktop < -0.9 && thisHourMultiplier_Desktop > -1) || thisHourMultiplier_Desktop > 3) {
        Logger.log("Desktop bid modifier '" + thisHourMultiplier_Desktop + "' for " + weekDays[dayOfWeek] + " " + hour + " is not valid.");
        thisHourMultiplier_Desktop = 0;
      }
 
      var totalMultiplier = ((1+thisHourMultiplier_Desktop)*(1+thisHourMultiplier))-1;
      sheet.getRange("I4").setValue(thisHourMultiplier_Desktop);
      sheet.getRange("T4").setValue(totalMultiplier);
      ModifyDesktopBidAdjustment(campaignIds, thisHourMultiplier_Desktop);
    }
  }
...
/**
* Function to set a tablet bid modifier for a set of campaigns
*
* @param array campaignIds An array of the campaign IDs to be affected
* @param Float bidModifier The multiplicative tablet bid modifier
* @return void
*/
function ModifyTabletBidAdjustment(campaignIds, bidModifier){
 
  var platformIds = [];
  var newBidModifier = Math.round(100*(1+bidModifier))/100;
 
  for(var i = 0; i < campaignIds.length; i++){
    platformIds.push([campaignIds[i],30002]);
  }
...
/**
* Function to set a desktop bid modifier for a set of campaigns
*
* @param array campaignIds An array of the campaign IDs to be affected
* @param Float bidModifier The multiplicative desktop bid modifier
* @return void
*/
function ModifyDesktopBidAdjustment(campaignIds, bidModifier){
 
  var platformIds = [];
  var newBidModifier = Math.round(100*(1+bidModifier))/100;
 
  for(var i = 0; i < campaignIds.length; i++){
    platformIds.push([campaignIds[i],30000]);
  }
